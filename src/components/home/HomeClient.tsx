...
export function HomeClient({ totalMembers, districtsCount }: { totalMembers: number; districtsCount: number }) {
  const { d } = useLang();
      ...
      <section className="border-b border-border bg-[linear-gradient(180deg,#FFFFFF_0%,#FFF8F0_100%)]">
        ...
        <div className="mx-auto max-w-[760px] px-4 py-8 text-center sm:px-8 sm:py-12">
          <h1 className="font-serif text-[20px] font-black leading-tight text-heading sm:text-[32px]">
            युवा शक्ति, राष्ट्र की शक्ति: <span className="text-primary-dark">BJYM Chhattisgarh</span> से जुड़ें!
          </h1>
          <p className="mx-auto mt-3 max-w-[560px] text-[13.5px] leading-relaxed text-muted sm:text-[15px]">
            भारतीय जनता युवा मोर्चा परिवार का हिस्सा बनें और भारत एवं छत्तीसगढ़ के नव-निर्माण में अपना योगदान दें।
          </p>

          <div className="mt-6 flex flex-col gap-2.5 px-2 sm:mt-7 sm:flex-row sm:flex-wrap sm:justify-center sm:gap-3 sm:px-0">
            <Link href="/register" className="w-full sm:w-auto"><Button size="lg" className="w-full sm:w-auto">{d.hero.ctaPrimary} →</Button></Link>
            <Link href="/login" className="w-full sm:w-auto"><Button variant="ghost" size="lg" className="w-full sm:w-auto">{d.nav.login}</Button></Link>
          </div>
          ...
          <div className="mt-7 inline-flex flex-wrap items-center justify-center gap-5 rounded-2xl border border-border bg-white/80 px-5 py-3.5 sm:mt-9 sm:gap-8 sm:px-8 sm:py-4">
            <div className="text-center">
              <div className="font-serif text-xl font-black text-primary-dark sm:text-2xl">{totalMembers.toLocaleString("en-IN")}</div>
              <div className="mt-0.5 text-[9px] font-extrabold uppercase tracking-wide text-muted sm:text-[10px]">{d.hero.statsMembers}</div>
            </div>
            <div className="text-center">
              <div className="font-serif text-xl font-black text-primary-dark sm:text-2xl">{districtsCount}</div>
              <div className="mt-0.5 text-[9px] font-extrabold uppercase tracking-wide text-muted sm:text-[10px]">{d.hero.statsDistricts}</div>
            </div>
            <div className="text-center">
              <div className="font-serif text-xl font-black text-primary-dark sm:text-2xl">QR</div>
              <div className="mt-0.5 text-[9px] font-extrabold uppercase tracking-wide text-muted sm:text-[10px]">{d.hero.statsVerify}</div>
            </div>
          </div>
        </div>
      </section>
